...
namespace Unity.Scenes
{
    ...
    class LiveLinkPlayerSystem : ComponentSystem
    {
        ...
        {
            #if !UNITY_EDITOR
            var editorHandshake = args.Receive<LiveLinkHandshake>();
            if (editorHandshake.LiveLinkId != LiveLinkUtility.LiveLinkId)
            {
            ...
            if (editorHandshake.LiveLinkCacheGUID != LiveLinkUtility.LiveLinkCacheGUID)
            {
                Debug.Log($"Editor LiveLink Cache GUID is different (Editor: {editorHandshake.LiveLinkCacheGUID.ToString()} != Player: {LiveLinkUtility.LiveLinkCacheGUID.ToString()}) - Clearing local LiveLink cache.");
                LiveLinkUtility.ClearPlayerLiveLinkCache();
                LiveLinkUtility.LiveLinkCacheGUID = editorHandshake.LiveLinkCacheGUID;
                LiveLinkUtility.WritePlayerLiveLinkCacheGUID();
            }
            #endif
