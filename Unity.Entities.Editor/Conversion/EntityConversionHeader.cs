        ...
        static void DisplayConvertToEntityHeaderCallBack(UnityEditor.Editor editor)
        {
            ...
            using (new EditorGUILayout.HorizontalScope(EditorStyles.largeLabel))
            {
                ...
                if (TargetsList.Count > 1)
                {
                    ...
                    {
                        if (gameObject.TryGetComponent<ConvertToEntity>(out var convertToEntityComponent))
                        {
                ...
                using (var changeScope = new EditorGUI.ChangeCheckScope())
                {
                    ...
                    if (changeScope.changed)
                    {
                        if (selectedGameObject.TryGetComponent<ConvertToEntity>(out var selectedConvertToEntityComponent))
                        {
                            Undo.DestroyObjectImmediate(selectedConvertToEntityComponent);
                        }
                        else
                        {
