...
namespace Unity.Entities
{
    class ComponentQueryCache
    {
        ...
        ComponentQueryBuilder[] m_CacheCheckQueryBuilders;
        int[][]                 m_CacheCheckDelegateTypeIndices;
        #endif

        public ComponentQueryCache(int cacheSize = 10)
        {
            #if ENABLE_UNITY_COLLECTIONS_CHECKS
            if (cacheSize <= 0)
            ...
            #if ENABLE_UNITY_COLLECTIONS_CHECKS
            m_CacheCheckQueryBuilders = new ComponentQueryBuilder[cacheSize];
        ...
        public unsafe int CreateCachedQuery(uint hash, ComponentGroup group
            #if ENABLE_UNITY_COLLECTIONS_CHECKS
            , ref ComponentQueryBuilder builder, int* delegateTypeIndices, int delegateTypeCount
            #endif
            )
        {
            #if ENABLE_UNITY_COLLECTIONS_CHECKS
            ...
            for (;;)
            {
                if (index == m_CacheHashes.Length)
                {
                    ...
                    UnityEngine.Debug.LogError(
                        $"{nameof(ComponentQueryCache)} is too small to hold the current number of queries this {nameof(ComponentSystem)} is running. The cache automatically expanded to {newSize}, " +
                        $"but this may cause a GC. Set cache size at init time via {nameof(ComponentSystem.InitComponentQueryCache)}() to a large enough number to ensure no allocations are required at run time.");
        ...
        #if ENABLE_UNITY_COLLECTIONS_CHECKS
        public unsafe void ValidateMatchesCache(int foundCacheIndex, ref ComponentQueryBuilder builder, int* delegateTypeIndices, int delegateTypeCount)
        {
