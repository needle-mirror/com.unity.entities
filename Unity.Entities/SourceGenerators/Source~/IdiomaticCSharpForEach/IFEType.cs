            ...
            $@"{(UseBurst ? $"[Unity.Burst.NoAlias]{Environment.NewLine}[{BurstCompileAttribute}]" : string.Empty)}
                ...
                {{
                    ...
                    {{
                        ...
                        {(UseBurst ? "if (Unity.Burst.CompilerServices.Hint.Unlikely(_currentEntityIndex >= _currentEntityCount))" : "if (_currentEntityIndex >= _currentEntityCount)")}
                        {{
                            {(UseBurst
                                ? "if (Unity.Burst.CompilerServices.Hint.Likely(_entityQueryEnumerator.MoveNextEntityRange(out bool movedToNewChunk, out ArchetypeChunk chunk, out int entityStartIndex, out int entityCount)))"
                                : "if (_entityQueryEnumerator.MoveNextEntityRange(out bool movedToNewChunk, out ArchetypeChunk chunk, out int entityStartIndex, out int entityCount))")}
                            {{
                                if (movedToNewChunk)
                                    _resolvedChunk = _typeHandle.Resolve(chunk);

                                _currentEntityIndex = entityStartIndex;
                                _currentEntityCount = entityCount;
                                return true;
                            }}
                            return false;
                        }}
