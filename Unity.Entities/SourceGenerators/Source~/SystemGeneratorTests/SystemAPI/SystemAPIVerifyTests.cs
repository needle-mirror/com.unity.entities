...
public unsafe partial struct NestedGetSingletonEntity : ISystem
{
    ...
    [TestMethod]
    public async Task SystemAPICallInPropertyDeclaration()
    {
        const string testSource = @"
using Unity.Entities;
using Unity.Entities.Tests;

public partial class SystemAPICallInPropertyDeclaration : SystemBase
{
    public Entity MyEntity_Property => SystemAPI.QueryBuilder().WithAll<EcsTestData>().Build().GetSingletonEntity();
    protected override void OnUpdate()
    {
    }
}";
        await VerifyCS.VerifySourceGeneratorAsync(testSource, nameof(SystemAPICallInPropertyDeclaration), "Test0__System_19875963020.g.cs");
    }
}
