    ...
    public class SystemApiQueryBuilderDescription
    {
        ...
        public SystemApiQueryBuilderDescription(SystemDescription systemDescription, QueryCandidate queryCandidate)
        {
                ...
                && invocationOperation.TargetMethod.ToString() == "Unity.Entities.SystemAPIQueryBuilder.Build()")
            {
                ...
                var containingMethodSyntax = queryCandidate.BuildNode.GetMethodSymbolSourceSyntaxOrDefault();
                if (containingMethodSyntax != null)
                {
                    var methodSymbol = SystemDescription.SemanticModel.GetMethodSymbol(containingMethodSyntax);
                    if (methodSymbol == null)
                        throw new InvalidOperationException($"Unable to get IMethodSymbol for containing method: {containingMethodSyntax}");
