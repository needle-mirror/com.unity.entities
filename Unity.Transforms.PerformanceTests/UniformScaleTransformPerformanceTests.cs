...
namespace Unity.Transforms.PerformanceTests
{
    ...
    public sealed unsafe partial class UniformScaleTransformPerformanceTests : ECSTestsFixture
    {
        [Test, Performance]
        public void UST_TransformTransform_Perf()
        {
            int count = 1000;
            ...
            var transforms =
                CollectionHelper.CreateNativeArray<UniformScaleTransform>(count, World.UpdateAllocator.ToAllocator);
            Measure.Method(() =>
                {
                    var t = transforms[0];
                    for (int i = 1; i < count; ++i)
                ...
                .SetUp(() =>
                {
                    for (int i = 0; i < count; ++i)
                    {
                        transforms[i] = UniformScaleTransform.FromPositionRotationScale(
                            rng.NextFloat3(),
        ...
        [Test, Performance]
        public void UST_InverseTransformTransform_Perf()
        {
            int count = 1000;
            var rng = new Random(17);
