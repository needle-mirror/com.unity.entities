...
{
    unsafe class ComponentQueryCacheTests
    {
        [Test]
        public void Ctor_WithCacheSize0_Throws()
        {
            // ReSharper disable ObjectCreationAsStatement
            Assert.Throws<ArgumentOutOfRangeException>(() => new ComponentQueryCache(0));
            Assert.Throws<ArgumentOutOfRangeException>(() => new ComponentQueryCache(-1));
            // ReSharper restore ObjectCreationAsStatement
        }

        static void SimpleWrapCreateCachedQuery(ComponentQueryCache cache, uint hash, ComponentGroup group)
        {
        ...
        {
            var cache = new ComponentQueryCache(1);
        ...
        {
            var cache = new ComponentQueryCache(2);
            SimpleWrapCreateCachedQuery(cache, 0, k_DummyGroup);
...
// TEMPORARY HACK
        ...
        {
            var cache = new ComponentQueryCache(4);
            SimpleWrapCreateCachedQuery(cache, 0, k_DummyGroup);
            SimpleWrapCreateCachedQuery(cache, 1, k_DummyGroup);
        ...
        public void ValidateMatchesCache_WithValidMatch_DoesNotThrow()
        {
            ...
            {
                var builder = new ComponentQueryBuilder().WithAll<EcsTestTag>();
                index = cache.CreateCachedQuery(0, k_DummyGroup, ref builder, delegateTypes, 1);
            }

            Assert.AreEqual(0, index);

            var testBuilder = new ComponentQueryBuilder().WithAll<EcsTestTag>();
        ...
        public void ValidateMatchesCache_WithMismatchedBuilder_Throws()
        {
            ...
            var anotherBuilder = new ComponentQueryBuilder();
